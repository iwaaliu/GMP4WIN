...
// Copyright (c) iwaa_liu 2004
// Use & modify as you want & need, and leave those 2 lines.
...
CGfxOutBarCtrl::CGfxOutBarCtrl()
{
	...
	pBlackPen = new CPen(PS_SOLID, 1, RGB(128,128,0));
...
BOOL CGfxOutBarCtrl::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
...
//	CRect inRc;
	...
	CMainFrame frm;
	CRdyh RdyhDlg;
	CSmoothSet SmoothDlg;
	CString cs, cs1;

	cs = CGfxOutBarCtrl::GetItemText(index);
	// cs1.Format("Clicked on %d - <%s>", (int)index, cs);
	// AfxMessageBox(cs1);	

	if(cs=="二度圆滑")  {if((RdyhDlg.DoModal())==IDOK)
						{
							frm.CallFunction();
							if(RdyhDlg.mopendata==1)
							{ShellExecute(NULL,"open","notepad.exe",RdyhDlg.m_output,NULL,SW_SHOW);}
						}
						}
	// else if(cs=="Game") {AfxMessageBox(cs1);}
	else if(cs=="平面圆滑")  {if((SmoothDlg.DoModal())==IDOK)
						{
							frm.CallFunction();
							if(SmoothDlg.mopendata==1)
							{ShellExecute(NULL,"open","notepad.exe",SmoothDlg.m_output,NULL,SW_SHOW);}
						}
						}
	else {/*AfxMessageBox(cs1)*/;}
